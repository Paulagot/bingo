...
pub fn handler<'info>(
    ...
    let winners_to_use: Vec<Pubkey> = if !ctx.accounts.room.winners.is_empty() {
        // Winners were declared via declare_winners instruction
        ctx.accounts.room.winners.clone()
    } else {
    ...
    match ctx.accounts.room.prize_mode {
        PrizeMode::PoolSplit => {
            // Original pool-based prize distribution
    ...
    // OPTIMIZATION: Move winners_to_use instead of cloning to reduce stack usage
    emit!(RoomEnded {
        room: room_key,
