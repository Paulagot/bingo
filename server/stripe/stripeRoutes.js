...
import { createCampaignStripeSession } from './campaignStripeCheckoutService.js';
...
stripeRouter.post('/walkin-checkout', async (req, res) => {
  try {
    ...
    if (!roomId || !playerName) return res.status(400).json({ error: 'roomId and playerName are required' });
    ...
    if (msg.includes('full') || msg.includes('capacity')) return res.status(409).json({ error: 'room_full', message: msg });
    if (msg === 'invalid_donation_amount_for_stripe') return res.status(400).json({ error: msg, message: 'Donation amount must be greater than 0.' });
    if (msg === 'invalid_checkout_amount') return res.status(400).json({ error: msg, message: 'Invalid checkout amount.' });
...
stripeRouter.post('/elimination-walkin-checkout', async (req, res) => {
  try {
    ...
    if (!roomId || !playerName) return res.status(400).json({ error: 'roomId and playerName are required' });
    const result = await createEliminationWalkinStripeSession({ roomId, playerName: playerName.trim(), appOrigin });
    return res.json({ ok: true, url: result.url, playerId: result.playerId });
  } catch (err) {
    const msg = err?.message || 'walkin_checkout_failed';
    console.error('[EliminationWalkinCheckout] ❌', msg);
    if (msg === 'stripe_not_ready_or_disabled' || msg === 'stripe_not_available_for_this_room') return res.status(422).json({ error: msg });
    if (msg === 'invalid_checkout_amount') return res.status(400).json({ error: msg });
    if (msg === 'Room not found') return res.status(404).json({ error: 'room_not_found' });
    return res.status(500).json({ error: 'walkin_checkout_failed', message: msg });
  }
});
...
stripeRouter.post('/campaign-checkout', async (req, res) => {
  try {
    ...
    if (!orderId || !campaignId) return res.status(400).json({ error: 'orderId and campaignId are required' });
    ...
    if (msg === 'order_not_payable') return res.status(400).json({ error: msg, message: 'Order is not in a payable state.' });
...
stripeRouter.post('/connect/reconnect',   reconnectStripeConnect);
stripeRouter.get('/connect/history',      getStripeHistory);