// server/puzzles/services/puzzleDropReconciliationService.js
//
// Period-aware reconciliation for Puzzle Drop, built on the SAME shared
...
// payment_method = 'stripe'. Drop entitlements confirm via cash, instant
// payment, Stripe, AND crypto (see createDropEntitlements/confirmDropPurchase
// in puzzleDropService.js), so this version sums confirmed 'entry_fee'
...
// Drop's ledger rows for a confirmed purchase can be cash, instant_payment,
// stripe, or crypto (see createDropEntitlements/confirmDropPurchase) — all
// of them count as real money received for this Drop, so all are summed.
...
export async function approveCurrentPeriod({ roomId, clubId, reconciliationId, approvedBy, notes }) {
  ...
  for (const a of adjustments) {
    switch (a.adjustmentType) {
      ...
      case 'refund':
      case 'fee':          moneyOut += a.amount; break;
