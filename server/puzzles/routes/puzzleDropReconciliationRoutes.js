...
//
// Period-aware reconciliation endpoints for Puzzle Drop.
// Mounted at /api/puzzle-drop-reconciliation - direct structural port of
...
} from '../services/puzzleDropReconciliationService.js';
...
async function getOwnedRoomClubId(roomId, req, res) {
  const [[room]] = await connection.execute(
    `SELECT club_id FROM ${ROOMS_TABLE} WHERE room_id = ? AND game_type = 'puzzle_drop' LIMIT 1`,
    [roomId]
  );
  if (!room) {
...
router.get('/room/:roomId/current', async (req, res) => {
  ...
  try {
    ...
    console.error('[DropRecon] GET current error:', err);
...
router.get('/room/:roomId/history', async (req, res) => {
  ...
  try {
    ...
    console.error('[DropRecon] GET history error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.get('/room/:roomId/summary', async (req, res) => {
  ...
  try {
    ...
    console.error('[DropRecon] GET summary error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/room/:roomId/adjustments', async (req, res) => {
  ...
  try {
    ...
    console.error('[DropRecon] POST adjustments error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.patch('/room/:roomId/adjustments/:id', async (req, res) => {
  ...
  try {
    ...
    console.error('[DropRecon] PATCH adjustment error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.delete('/room/:roomId/adjustments/:id', async (req, res) => {
  ...
  try {
    ...
    console.error('[DropRecon] DELETE adjustment error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/room/:roomId/approve', async (req, res) => {
  ...
  try {
    ...
    console.error('[DropRecon] POST approve error:', err);
