...
async function getOwnedRoomClubId(roomId, req, res) {
  const [[room]] = await connection.execute(
    `SELECT club_id FROM ${ROOMS_TABLE} WHERE room_id = ? AND game_type = 'sponsored_activity' LIMIT 1`,
    [roomId]
  );
  if (!room) {
...
router.get('/room/:roomId/current', async (req, res) => {
  ...
  try {
    ...
    console.error('[SponsoredRecon] GET current error:', err);
...
router.get('/room/:roomId/history', async (req, res) => {
  ...
  try {
    ...
    console.error('[SponsoredRecon] GET history error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.get('/room/:roomId/summary', async (req, res) => {
  ...
  try {
    ...
    console.error('[SponsoredRecon] GET summary error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/room/:roomId/adjustments', async (req, res) => {
  ...
  try {
    ...
    const insertedId = await addAdjustment({
      ...
      note: note ?? null,
      createdBy: createdBy ?? req.user?.name ?? req.user?.email ?? req.club_id ?? 'Admin',
    });
    ...
    console.error('[SponsoredRecon] POST adjustments error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.patch('/room/:roomId/adjustments/:id', async (req, res) => {
  ...
  try {
    ...
    console.error('[SponsoredRecon] PATCH adjustment error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.delete('/room/:roomId/adjustments/:id', async (req, res) => {
  ...
  try {
    ...
    console.error('[SponsoredRecon] DELETE adjustment error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/room/:roomId/approve', async (req, res) => {
  ...
  try {
    ...
    console.error('[SponsoredRecon] POST approve error:', err);
