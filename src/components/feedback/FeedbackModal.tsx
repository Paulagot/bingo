...
//
// Auto-shows at game over. Player can dismiss (skip) or answer and submit.
// Props are intentionally minimal so it's easy to drop into any game screen.
//
// Usage (Quiz):
//   <FeedbackModal roomId={roomId} clubId={clubId} gameType="quiz" />
//
// Usage (Elimination):
//   <FeedbackModal roomId={roomId} clubId={clubId} gameType="elimination" />
...
interface FeedbackModalProps {
  ...
  gameType?: string;
  /** Called when the modal closes (either submitted or dismissed) */
  onClose?:  () => void;
}
...
const QUESTIONS: {
  key: 'enjoyed_game' | 'play_again' | 'recommend';
  text: string;
  emoji: string;
}[] = [
...
export function FeedbackModal({ roomId,  gameType, onClose }: FeedbackModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    // After successful submit, auto-close after a short pause
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-4 backdrop-blur-sm sm:items-center">
      <div className="w-full max-w-sm rounded-2xl bg-gradient-to-br from-indigo-900 to-indigo-800 p-6 shadow-2xl">
        ...
        <div className="mb-5 flex items-start justify-between">
          ...
          <button
            onClick={handleClose}
            className="ml-4 rounded-lg p-1.5 text-indigo-400 transition hover:bg-white/10 hover:text-white"
            aria-label="Dismiss"
            type="button"
          >
            ...
            <div key={key}>
              ...
              <div className="flex gap-2">
                <YesNoButton
                  label="👍 Yes"
                  value={true}
                  selected={feedback.answers[key]}
                  onClick={(v) => feedback.setAnswer(key, v)}
                />
                <YesNoButton
                  label="👎 No"
                  value={false}
        ...
        {feedback.errorMessage && (
          <p className="mt-3 rounded-lg bg-rose-500/20 px-3 py-2 text-xs text-rose-300">
            {feedback.errorMessage}
          </p>
        )}
        ...
        <div className="mt-6 flex gap-3">
          <button
            onClick={handleClose}
            className="flex-1 rounded-xl border border-white/20 py-3 text-sm text-white/60 transition hover:bg-white/10 hover:text-white"
            type="button"
          >
            Skip
          </button>
