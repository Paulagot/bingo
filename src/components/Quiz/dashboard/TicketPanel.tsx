// src/components/Quiz/dashboard/TicketsPanel.tsx
...
export const TicketsPanel: React.FC<TicketsPanelProps> = ({ roomId, confirmer }) => {
  ...
  const loadTickets = useCallback(async () => {
    try {
      ...
      console.error('[TicketsPanel] Failed to load tickets:', err);
  ...
  const confirmPayment = async (ticketId: string) => {
    try {
      ...
      if (!confirmer?.id || !confirmer?.name || !confirmer?.role) {
        throw new Error('Missing confirmer identity. Please ensure you are logged in as host or admin.');
      }
      ...
      const response = await fetch(`/api/quiz/tickets/${ticketId}/confirm`, {
        ...
        body: JSON.stringify({
          ...
          adminNotes: 'Confirmed from Host Dashboard',
        }),
      });
      ...
      console.error('[TicketsPanel] Failed to confirm payment:', err);
        ...
        {confirmer && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <p className="text-xs text-blue-800">
              <strong>Note:</strong> Viewing as {confirmer.name} ({confirmer.role}).
              You can confirm payments and show ticket QR codes from this panel. This list updates automatically.
            </p>
          </div>
        )}
      </div>
