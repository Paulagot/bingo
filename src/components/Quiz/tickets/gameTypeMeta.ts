...
//
// Centralises all game-type-specific copy, URL patterns,
// and payment-reference prefixes for the ticket UI.
//
// Import this in TicketPurchaseFlow, TicketConfirmation,
// and TicketStatusChecker instead of scattering game-type
// if/else checks through every component.
...
export interface GameTypeMeta {
  ...
  // Prefix used when creating a manual / instant-payment reference.
  // Examples:
  //   QUIZ-ABC123
  //   ELIM-ABC123
  //   EVENT-ABC123
...
const META: Record<GameType, GameTypeMeta> = {
  ...
  ticketed_event: {
    ...
    joinLabel: 'View Ticket',
    buyingLabel: 'event',
    completedLabel: 'event',

    paymentReferencePrefix: 'EVENT',
...
export function getGameTypeMeta(
  gameType?: string | null
): GameTypeMeta {
  if (gameType === 'elimination') {
    return META.elimination;
  }

  if (gameType === 'ticketed_event') {
    return META.ticketed_event;
  }

  // Default remains quiz for backwards compatibility with
  // older ticket records / room responses that may not have gameType.
  return META.quiz;
}