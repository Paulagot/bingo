// src/components/Quiz/tickets/crypto/CryptoTicketDonationStep.tsx
...
} from '../../joinroom/crypto/useSolanaDirectDonation';

import type { ClubPaymentMethod } from '../../shared/PaymentMethodSelector';
import type { Ticket } from '../types';

interface CryptoTicketDonationStepProps {
  roomId: string;
...
interface CryptoTicketConfirmResponse {
  ok: boolean;
  error?: string;
  ticket?: Ticket;
  ticketId?: string;
...
export const CryptoTicketDonationStep: React.FC<CryptoTicketDonationStepProps> = ({
  roomId,
  purchaserName,
  purchaserEmail,
  purchaserPhone = null,
  ...
  const [confirmData, setConfirmData] = useState<CryptoTicketConfirmResponse | null>(null);
  ...
  const handleOpenWalletOptions = async () => {
    try {
      ...
      console.error('[CryptoTicketDonationStep] wallet options failed:', err);
  ...
  const confirmCryptoTicketOnBackend = async (
    ...
    const response = await fetch('/api/quiz/tickets/crypto-donation/confirm', {
      ...
      body: JSON.stringify({
        ...
        playerName: playerName?.trim() || purchaserName.trim(),
    ...
    let data: CryptoTicketConfirmResponse;
    ...
    try {
      ...
      data = {
        ok: false,
        error: 'Server returned an invalid response while confirming the crypto ticket payment.',
      };
    }
    ...
    if (!response.ok || !data.ok) {
      throw new Error(
        data?.error ||
          'Your transaction was sent, but the server could not create the ticket. Please contact the host with your transaction signature.'
      );
    }

    if (!data.ticket) {
      throw new Error('Crypto payment was confirmed, but no ticket was returned.');
    }
  ...
  const handleConnectWalletAndPay = async () => {
    try {
      ...
      const backendConfirm = await confirmCryptoTicketOnBackend(result);

      setConfirmData(backendConfirm);
      setStatus('success');

      window.setTimeout(() => {
        onComplete(backendConfirm.ticket as Ticket);
      }, 700);
    } catch (err: any) {
      console.error('[CryptoTicketDonationStep] payment failed:', err);
      setStatus('idle');
      setError(err?.message || 'Crypto ticket payment failed.');
    }
  };
  ...
  const buttonLabel = (() => {
    ...
    if (status === 'confirming') return 'Creating Ticket…';
    if (status === 'success') return 'Ticket Created';
  ...
  return (
    ...
    >
      <div className="max-h-[70vh] overflow-y-auto pr-1 sm:max-h-none sm:overflow-visible sm:pr-0">
        <div className="space-y-3 sm:space-y-4">
          <div className="rounded-lg border border-purple-200 bg-purple-50 p-3 sm:p-4">
            <div className="font-semibold text-purple-950">
              Pay for your ticket with a Solana wallet
            </div>
            <p className="mt-1 text-sm text-purple-900">
              Choose a token and amount. After the blockchain payment is verified, your ticket will be ready immediately.
            </p>
          </div>
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900 sm:p-4 sm:text-sm">
            Your ticket is only created after the transaction is verified. Always check the token,
...
export default CryptoTicketDonationStep;