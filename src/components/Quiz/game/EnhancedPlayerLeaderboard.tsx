...
// Enhanced FloatingExtrasBar Component with mobile-friendly two-tap tooltips
const FloatingExtrasBar: React.FC<{
  ...
  // Get actual extra definitions, already pre-filtered by the hook
  const globalExtraDefinitions = globalExtras
    .map(
      (extraId) =>
        fundraisingExtraDefinitions[
          extraId as keyof typeof fundraisingExtraDefinitions
        ],
    )
    .filter(Boolean);
  ...
  const handleExtraClick = (extraId: string) => {
     ...
     */
    if (touchDevice && activeTooltipId !== extraId) {
      showTooltip(extraId);
      return;
    }
  ...
  return (
    <>
      <style>{`
        ...
        /* Keyboard + mobile active/focus effect */
        .floating-power-up:focus-visible:not(:disabled),
        .floating-power-up.tooltip-open:not(:disabled),
        .floating-power-up:active:not(:disabled) {
        ...
        .floating-power-up:focus .floating-power-up-tooltip,
        .floating-power-up:focus-visible .floating-power-up-tooltip,
        .floating-power-up.tooltip-open .floating-power-up-tooltip {
          opacity: 1;
          visibility: visible;
          transform: translateX(-50%) translateY(-2px);
        }
      ...
      <div className="fixed bottom-6 left-1/2 z-40 -translate-x-1/2 transform">
        <div className="enhanced-floating-bar">
          <div className="flex items-center space-x-4">
            <span className="floating-bar-label">Take Action:</span>
            ...
            {globalExtraDefinitions.map((extra) => {
              ...
              const isTooltipOpen = activeTooltipId === extra.id;
              ...
              return (
                <button
                  ...
                  onFocus={() => {
                    if (!isTouchDevice()) {
                      showTooltip(extra.id);
                    }
                  }}
                  onBlur={() => hideTooltip(extra.id)}
                  ...
                  aria-label={
                    isTouchDevice() && !isTooltipOpen
                      ? `${powerUpInfo.tooltip}. Tap once to preview, tap again to use.`
                      : powerUpInfo.tooltip
                  }
                >
                  {powerUpInfo.icon}
                  ...
                  {shouldDisable && <div className="disabled-indicator">✗</div>}

                  {/* Enhanced tooltip */}
                  <div className="floating-power-up-tooltip" role="tooltip">
...
const EnhancedPlayerLeaderboard: React.FC<EnhancedPlayerLeaderboardProps> = ({
  ...
  const currentPlayerPosition = leaderboard.findIndex(
  ...
  return (
    <div className="relative">
      ...
      <div
        className={`bg-muted overflow-hidden rounded-xl border-2 shadow-lg transition-all duration-500 ${
          isRoundResults
      ...
      >
        ...
        >
          <div className="text-center">
            ...
            <p className={isRoundResults ? 'text-purple-700' : 'text-green-700'}>
              {isRoundResults
              ...
              <div className="bg-muted/70 mt-3 rounded-lg p-3">
                <div className="flex items-center justify-center space-x-2">
                  ...
                  <span className="font-semibold text-purple-800">
                    {leaderboard[0]?.name ?? 'Someone'} won Round{' '}
                    {currentRound ?? '?'}!
                  </span>
          ...
          <div className="space-y-3">
            {displayedLeaderboard
              ...
              .map((entry, displayIdx) => {
                ...
                const actualPosition = leaderboard.findIndex(
                  (p) => p.id === entry.id,
                );
                const isWinner = actualPosition === 0;
                const isCurrentPlayerOutsideTop10 =
                  currentPlayerPosition >= 10 && isCurrentPlayer;

                return (
                  <div key={entry.id}>
                    {/* Separator for current player if outside top 10 */}
                    {isCurrentPlayerOutsideTop10 &&
                      displayIdx === displayedLeaderboard.length - 1 && (
                        <div className="my-4 flex items-center">
                          <div className="flex-1 border-t border-gray-300" />
                          <span className="text-fg/60 bg-muted px-3 text-sm">
                            Your Position
                          </span>
                          <div className="flex-1 border-t border-gray-300" />
                        </div>
                    ...
                    <div
                      className={`flex items-center justify-between rounded-lg border-2 p-4 transition-all duration-200 ${
                        ...
                        isRoundResults && actualPosition <= 2
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {actualPosition === 1 && (
                            <Medal className="text-fg/70 h-5 w-5" />
                          )}
                      ...
                      <div className="text-right">
                        ...
                        {isCurrentPlayer &&
                          (cumulativeNegativePoints > 0 || pointsRestored > 0) && (
                            <div
                              className={`text-xs ${
                                isRoundResults
                                  ? 'text-purple-600'
                            ...
                            >
                              {cumulativeNegativePoints > 0 && (
                                <div>Lost: -{cumulativeNegativePoints}</div>
                              )}
                              {pointsRestored > 0 && (
                                <div>Restored: +{pointsRestored}</div>
                              )}
                            </div>
                          )}
                      </div>
