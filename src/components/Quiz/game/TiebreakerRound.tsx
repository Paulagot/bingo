...
const TiebreakerRound: React.FC<TiebreakerRoundProps> = ({
  ...
  // Timer setup matching ModernStandardRound
...
const { timeLeft } = useQuizTimer({
  ...
  timerActive: !hasSubmitted && isTieBreakerParticipant && !!timerQuestion, // Fixed logic
...
useEffect(() => {
  console.log('[TiebreakerRound] Timer state:', {
    timeLeft,
    timerActive: timerActive && !hasSubmitted && isTieBreakerParticipant,
    timerQuestion,
    hasSubmitted,
    isTieBreakerParticipant
  });
  ...
  // Calculate distances for review
      ...
      <style>{`
        ...
        .tiebreaker-card {
          ...
          background-image: linear-gradient(135deg, #fffbeb 0%, #fef3c7 100%), 
                           linear-gradient(45deg, #f59e0b, #d97706, #fbbf24, #f59e0b);
          background-origin: border-box;
          background-clip: padding-box, border-box;
          animation: goldBorderFlow 3s ease-in-out infinite;
        }
          ...
          {!question ? (
            <div className="participants-info">
              ...
              <div className="participants-list">
                Participants: {(tbStillTied.length ? tbStillTied : tbParticipants).map(getPlayerName).join(', ')}
              </div>
            </div>
          ) : showReview ? (
            // Review Phase
              ...
              {correctAnswer !== undefined && (
                <div className="correct-answer">
                  <div className="correct-answer-text">
                    Correct Answer: {correctAnswer.toLocaleString()}
                  </div>
                </div>
              )}
              ...
              {tbWinners && tbWinners.length > 0 && (
                <div className="winners-announcement">
                  ...
                  <div className="next-round-text">
                    {tbWinners.length === 1 ? 'Returning to final results...' : 'Another tie-breaker needed...'}
                  </div>
                </div>
              )}
            </div>
