...
import puzzleDropReconciliationService, {
  type DropAdjustment,
  type DropReconciliationPeriod,
  type DropLifetimeSummary,
} from '../../../../services/PuzzleDropReconciliationService';
import { DropAdjustmentsLedger } from './DropAdjustmentsLedger';
...
export default function DropReconciliationTab({ roomId, currencySymbol = '€', hostName = 'Host' }: Props) {
  const [summary, setSummary]             = useState<DropLifetimeSummary | null>(null);
  const [current, setCurrent]             = useState<DropReconciliationPeriod | null>(null);
  const [adjustments, setAdjustments]     = useState<DropAdjustment[]>([]);
  const [liveReceipts, setLiveReceipts]   = useState<{ total: number; count: number }>({ total: 0, count: 0 });
  const [history, setHistory]             = useState<DropReconciliationPeriod[]>([]);
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [currentRes, summaryRes] = await Promise.all([
        puzzleDropReconciliationService.getCurrent(roomId),
        puzzleDropReconciliationService.getSummary(roomId),
      ]);
  ...
  const loadHistory = async () => {
    ...
    if (!history.length) {
      try {
        const res = await puzzleDropReconciliationService.getHistory(roomId);
        setHistory([...res.history].reverse());
      } catch (e) {
        console.error('[DropReconciliationTab] history load failed:', e);
      }
    }
  };
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await puzzleDropReconciliationService.approve(roomId, {
