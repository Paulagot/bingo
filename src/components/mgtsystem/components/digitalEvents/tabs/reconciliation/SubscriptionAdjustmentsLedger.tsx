...
interface Props {
  ...
  isLocked: boolean;
  approverName: string;
  onChange: (next: SubscriptionAdjustment[]) => void;
}

export const SubscriptionAdjustmentsLedger: React.FC<Props> = ({
  ...
  const handleAdd = async () => {
    ...
    setAddingNew(true);
    try {
      const res = await subscriptionReconciliationService.addAdjustment(roomId, {
        adjustmentType: 'fee',
        amount: 0,
      ...
      console.error('[SubscriptionAdjustmentsLedger] add failed:', e);
    } finally {
      setAddingNew(false);
    }
  };
  ...
  const handleSelectChange = async (id: string, field: string, value: string) => {
    if (isLocked) return;
    let patch: Partial<SubscriptionAdjustment> = { [field]: value } as any;
    ...
    try {
      await subscriptionReconciliationService.updateAdjustment(roomId, id, patch as any);
    } catch (e) {
      console.error('[SubscriptionAdjustmentsLedger] update failed:', e);
    } finally {
  ...
  const handleBlurAmount = async (id: string, amount: number) => {
    ...
    setRowSaving(id, true);
    try {
      await subscriptionReconciliationService.updateAdjustment(roomId, id, { amount });
    } catch (e) {
      console.error('[SubscriptionAdjustmentsLedger] amount update failed:', e);
    } finally {
      setRowSaving(id, false);
    }
  };
  ...
  const handleBlurNote = async (id: string, note: string) => {
    ...
    setRowSaving(id, true);
    try {
      await subscriptionReconciliationService.updateAdjustment(roomId, id, { note });
    } catch (e) {
      console.error('[SubscriptionAdjustmentsLedger] note update failed:', e);
    } finally {
      setRowSaving(id, false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    setRowSaving(id, true);
    try {
      await subscriptionReconciliationService.deleteAdjustment(roomId, id);
      onChange(adjustments.filter(a => a.id !== id));
    } catch (e) {
      console.error('[SubscriptionAdjustmentsLedger] delete failed:', e);
    } finally {
      setRowSaving(id, false);
    }
  };
  ...
  const totals = useMemo(() => {
    ...
    for (const a of adjustments) {
      ...
      else moneyOut += a.amount; // refund / fee
    }
    ...
    <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
      <div className="p-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            {isLocked ? 'Locked after approval' : 'Refunds and Stripe fees for the current period'}
          </p>
        </div>
      ...
      {adjustments.length === 0 ? (
        <div className="p-8 text-center text-gray-400">
          ...
          <p className="text-xs mt-1">Record a Stripe fee, refund, or correction if needed</p>
        </div>
      ) : (
