// src/components/mgtsystem/components/digitalEvents/tabs/reconciliation/SponsoredActivityReconciliationTab.tsx
//
...
// model. Sponsored Activity doesn't have a recurring billing cycle the way
// subscriptions do, but it CAN be on sale indefinitely, so the same
// "open a period, add adjustments, approve, next period opens" workflow
// applies for regular financial sign-off even without a natural billing
// boundary triggering it.
...
import sponsoredActivityReconciliationService, {
  ...
  type SponsoredActivityReconciliationPeriod,
  type SponsoredActivityLifetimeSummary,
} from '../../../../services/SponsoredActivityReconciliationService';
import { SponsoredActivityAdjustmentsLedger } from './SponsoredActivityAdjustmentsLedger';
...
export default function SponsoredActivityReconciliationTab({ roomId, currencySymbol = '€', hostName = 'Host' }: Props) {
  const [summary, setSummary]             = useState<SponsoredActivityLifetimeSummary | null>(null);
  const [current, setCurrent]             = useState<SponsoredActivityReconciliationPeriod | null>(null);
  const [adjustments, setAdjustments]     = useState<SponsoredActivityAdjustment[]>([]);
  const [liveReceipts, setLiveReceipts]   = useState<{ total: number; count: number }>({ total: 0, count: 0 });
  const [history, setHistory]             = useState<SponsoredActivityReconciliationPeriod[]>([]);
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [currentRes, summaryRes] = await Promise.all([
        sponsoredActivityReconciliationService.getCurrent(roomId),
        sponsoredActivityReconciliationService.getSummary(roomId),
      ]);
  ...
  const loadHistory = async () => {
    ...
    if (!history.length) {
      try {
        const res = await sponsoredActivityReconciliationService.getHistory(roomId);
        setHistory([...res.history].reverse());
      } catch (e) {
        console.error('[SponsoredActivityReconciliationTab] history load failed:', e);
      }
    }
  };
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await sponsoredActivityReconciliationService.approve(roomId, {
        approvedBy: approvedBy.trim(),
        notes: notes.trim() || null,
      });
      setCurrent(res.reconciliation);
      setApproveOk(true);
      const summaryRes = await sponsoredActivityReconciliationService.getSummary(roomId);
      setSummary(summaryRes.summary);
    } catch (e: any) {
  ...
  const projectedClosing = current.openingBalance + liveReceipts.total +
    adjustments.reduce((net, a) => {
      if (a.adjustmentType === 'received') return net + a.amount;
  ...
  return (
    <div className="space-y-6 p-5">
      ...
      {/* ── Lifetime summary ──────────────────────────────────────────────── */}
      {summary && (
        <div className="rounded-2xl border border-[rgba(21,127,133,0.25)] bg-[rgba(21,127,133,0.05)] p-5">
          <div className="mb-3 flex items-center gap-2">
            <TrendingUp className="h-4 w-4 text-[#157f85]" />
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-[#157f85]">{fmt(currencySymbol, summary.currentBalance)}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ── Current period ─────────────────────────────────────────────────── */}
      <div className={`rounded-xl border p-5 shadow-sm ${isApproved ? 'border-green-200 bg-white' : 'border-[#157f85] bg-white'}`}>
        <div className="mb-4 flex items-center gap-3">
          <Scale className="h-5 w-5 text-[#157f85]" />
        ...
        <div className="mb-5 flex flex-wrap items-center gap-3">
          ...
          <div className={`rounded-xl border px-4 py-3 ${
            (isApproved ? current.adjustmentsNet : adjustments.reduce((n, a) => {
              if (a.adjustmentType === 'received') return n + a.amount;
              if (a.adjustmentType === 'cash_over_short') return n + (a.reasonCode === 'cash_over' ? a.amount : -a.amount);
              return n - a.amount;
            }, 0)) >= 0
          ...
          <ArrowRight className="h-4 w-4 text-gray-300 shrink-0" />
          <div className="rounded-xl bg-gradient-to-r from-[#157f85] to-[#a855f7] px-4 py-3 text-white">
        ...
        <SponsoredActivityAdjustmentsLedger
          roomId={roomId}
            ...
            <div className="space-y-3 rounded-xl border border-gray-200 bg-gray-50 p-4">
              ...
              <div>
                ...
                <input type="text" value={approvedBy} onChange={e => setApprovedBy(e.target.value)}
                  ...
                  className="w-full rounded-lg border-2 border-gray-300 px-3 py-2 text-sm focus:border-[#157f85] focus:outline-none focus:ring-2 focus:ring-[rgba(21,127,133,0.2)] disabled:bg-gray-100" />
              </div>
              <div>
                ...
                <textarea rows={2} value={notes} onChange={e => setNotes(e.target.value)}
                  ...
                  className="w-full rounded-lg border-2 border-gray-300 px-3 py-2 text-sm focus:border-[#157f85] focus:outline-none focus:ring-2 focus:ring-[rgba(21,127,133,0.2)] disabled:bg-gray-100 resize-none" />
              </div>
              <button onClick={handleApprove} disabled={!approvedBy.trim() || approving}
                className="w-full rounded-xl bg-[#157f85] py-3 text-sm font-bold text-white hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2">
                {approving
