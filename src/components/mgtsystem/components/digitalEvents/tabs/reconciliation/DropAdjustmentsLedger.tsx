...
interface Props {
  ...
  isLocked: boolean;
  approverName: string;
  onChange: (next: DropAdjustment[]) => void;
}

export const DropAdjustmentsLedger: React.FC<Props> = ({
  ...
  const handleAdd = async () => {
    ...
    setAddingNew(true);
    try {
      const res = await puzzleDropReconciliationService.addAdjustment(roomId, {
        adjustmentType: 'fee',
        amount: 0,
        paymentMethod: 'other',
      ...
      console.error('[DropAdjustmentsLedger] add failed:', e);
    } finally {
      setAddingNew(false);
    }
  };
  ...
  const handleSelectChange = async (id: string, field: string, value: string) => {
    if (isLocked) return;
    let patch: Partial<DropAdjustment> = { [field]: value } as any;
    ...
    try {
      await puzzleDropReconciliationService.updateAdjustment(roomId, id, patch as any);
    } catch (e) {
      console.error('[DropAdjustmentsLedger] update failed:', e);
    } finally {
  ...
  const handleBlurAmount = async (id: string, amount: number) => {
    ...
    setRowSaving(id, true);
    try {
      await puzzleDropReconciliationService.updateAdjustment(roomId, id, { amount });
    } catch (e) {
      console.error('[DropAdjustmentsLedger] amount update failed:', e);
    } finally {
      setRowSaving(id, false);
    }
  };
  ...
  const handleBlurNote = async (id: string, note: string) => {
    ...
    setRowSaving(id, true);
    try {
      await puzzleDropReconciliationService.updateAdjustment(roomId, id, { note });
    } catch (e) {
      console.error('[DropAdjustmentsLedger] note update failed:', e);
    } finally {
      setRowSaving(id, false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    setRowSaving(id, true);
    try {
      await puzzleDropReconciliationService.deleteAdjustment(roomId, id);
      onChange(adjustments.filter(a => a.id !== id));
    } catch (e) {
      console.error('[DropAdjustmentsLedger] delete failed:', e);
    } finally {
      setRowSaving(id, false);
    }
  };
