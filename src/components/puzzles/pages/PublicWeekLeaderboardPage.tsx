...
//
// Public full leaderboard for one weekly puzzle.
// Updated to use the shared PuzzleTypePreview component.
...
export default function PublicWeekLeaderboardPage() {
  ...
  const topThree = useMemo(() => board?.entries.slice(0, 3) ?? [], [board]);
  const remaining = useMemo(() => board?.entries.slice(3) ?? [], [board]);

  async function handleShare() {
    if (!board) return;

    const shareUrl = window.location.href;
    const puzzleLabel = getPuzzleTypeLabel(board.puzzleType);
    ...
    try {
      ...
      if ((err as DOMException)?.name !== 'AbortError') {
        console.warn('[PublicWeekLeaderboardPage] Share failed:', err);
      }
    }
  }
    ...
    return (
      <PuzzlePageShell theme={theme}>
        <div className="mx-auto max-w-xl rounded-[28px] border border-rose-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm text-[#6E6A63]">{pageError ?? 'This week has no public leaderboard.'}</p>
        </div>
      </PuzzlePageShell>
    );
  }
  ...
  const puzzleLabel = getPuzzleTypeLabel(puzzleType);
      ...
      <div className="mx-auto w-full min-w-0 max-w-5xl overflow-x-hidden">
        <section className="grid min-w-0 gap-5 overflow-hidden rounded-[28px] border border-[#E8E0D3] bg-white p-5 shadow-sm sm:p-7 lg:grid-cols-[1fr_0.8fr] lg:items-center">
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#E36B2C] sm:text-xs">Week {board.weekNumber} leaderboard</p>
            <h1 className="mt-2 break-words font-serif text-[2.4rem] leading-[1] text-[#071A44] sm:text-5xl">{puzzleLabel}</h1>
            <p className="mt-2 break-words text-sm leading-6 text-[#5F5A54] sm:text-base">{challenge.title}</p>

            <div className="mt-4 flex min-w-0 flex-wrap gap-2">
              <InfoPill label={`${difficulty} difficulty`} />
              <InfoPill label={`${entries.length} player${entries.length !== 1 ? 's' : ''}`} />
              <InfoPill label={isFinal ? 'Final results' : 'Board still open'} />
            </div>
          ...
          <div className="min-w-0 rounded-[24px] bg-[linear-gradient(135deg,#FBF8F3_0%,var(--puzzle-bg-accent)_100%)] p-4 sm:p-5">
            <div className="rounded-[20px] bg-white p-4 shadow-sm">
              <PuzzleTypePreview puzzleType={puzzleType} size="hero" />
            </div>
          </div>
        </section>
        ...
        {entries.length === 0 ? (
          <section className="mt-5 rounded-[28px] border border-dashed border-[#D8D1C4] bg-white p-8 text-center shadow-sm">
            ...
            <h2 className="mt-4 font-serif text-3xl text-[#071A44]">The podium is wide open</h2>
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#6E6A63]">No one has posted a score yet.</p>
          </section>
        ) : (
          <>
            <section className="mt-5 rounded-[28px] border border-[#E8E0D3] bg-white p-5 shadow-sm sm:p-7">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#8A847B] sm:text-xs">Top players</p>
                <h2 className="mt-1 font-serif text-3xl text-[#071A44]">This week’s podium</h2>
              </div>

              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                {topThree.map(entry => (
                  <PodiumCard key={`${entry.rank}-${entry.playerName}`} entry={entry} />
                ))}
              </div>
            </section>
...
function InfoPill({ label }: { label: string }) {
  return <span className="rounded-full border border-[#E8E0D3] bg-[#FBF8F3] px-3 py-1.5 text-[10px] font-bold capitalize text-[#6E6A63] sm:text-xs">{label}</span>;
}

function PodiumCard({ entry }: { entry: WeekLeaderboard['entries'][number] }) {
  const rankStyle =
    entry.rank === 1 ? 'bg-[#FFF2D9] border-[#F3D79B]' : entry.rank === 2 ? 'bg-[#EEF3FB] border-[#D6E2F2]' : 'bg-[#FBEFDF] border-[#EFCFAE]';

  return (
    <article className={`min-w-0 rounded-[24px] border p-4 ${rankStyle}`}>
      <div className="flex items-center justify-between gap-3">
        ...
        <TrophyIcon className={entry.rank === 1 ? 'h-6 w-6 text-[#8A5A00]' : 'h-5 w-5 text-[#6E6A63]'} />
      </div>

      <h3 className="mt-4 truncate text-base font-black text-[#071A44]">{entry.playerName}</h3>
      <p className="mt-1 text-xs font-semibold text-[#6E6A63]">{entry.isCorrect ? 'Solved' : 'Attempted'}</p>

      <div className="mt-4 flex flex-wrap gap-2">
        {entry.timeTakenSeconds !== null ? <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-bold text-[#6E6A63]">{formatDuration(entry.timeTakenSeconds)}</span> : null}
        <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-black text-[#071A44]">{entry.totalScore} pts</span>
      </div>
    </article>
  );
}
...
function ShareIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
      <path d="M12 3v11m0-11 4 4m-4-4L8 7M5 10v8a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-8" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
