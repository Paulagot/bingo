...
// Polished subscriber-facing layout aligned with PlayerChallengePage.
...
export default function PlayerOverallLeaderboardPage() {
  ...
  const [challenge, setChallenge] =
    useState<PublicChallenge | null>(null);

  const [entries, setEntries] =
    useState<SupporterLeaderboardEntry[]>([]);
    ...
    async function load() {
      ...
      try {
        const [
          ...
          supporterAuthService.getPublicChallenge(
            currentChallengeId
          ),
          supporterAuthService.getOverallLeaderboard(
            currentChallengeId
          ),
          supporterAuthService
            .getMe()
  ...
  const myEntry = useMemo(
    () =>
      entries.find(
        entry =>
          myPlayerId != null &&
          String(entry.playerId) === myPlayerId
      ) ?? null,
    [entries, myPlayerId]
  );
      ...
      <PuzzlePageShell theme={theme}>
        <div className="mx-auto max-w-xl rounded-[28px] border border-rose-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm text-[#6E6A63]">
            {pageError ??
              'Could not load your standing for this challenge.'}
          </p>
      ...
      <div className="mx-auto w-full min-w-0 max-w-5xl overflow-x-hidden">
        ...
        <section className="rounded-[28px] border border-[#E8E0D3] bg-white p-5 shadow-sm sm:p-7">
          ...
          <p className="mt-3 max-w-2xl break-words text-sm leading-6 text-[#5F5A54] sm:text-base">
            Your cumulative score across every puzzle
            you’ve played.
          </p>
          ...
          {myEntry ? (
            <div className="mt-6 grid min-w-0 gap-3 sm:grid-cols-3">
              ...
              <StandingStat
                label="Total score"
                value={`${myEntry.totalScore}`}
              />
              <StandingStat
                label="Puzzles completed"
                value={`${myEntry.weeksCompleted}`}
              />
            </div>
          ) : null}
        </section>
        ...
        {entries.length === 0 ? (
          <section className="mt-5 rounded-[28px] border border-dashed border-[#D8D1C4] bg-white p-8 text-center shadow-sm">
            ...
            <p className="mx-auto mt-2 max-w-md text-sm text-[#6E6A63]">
              Standings appear once players begin
              submitting puzzles.
            </p>
          </section>
        ) : (
...
function StandingStat({
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-[#E8E0D3] bg-[#FBF8F3] px-4 py-4">
      ...
      <p className="mt-1 font-serif text-3xl text-[#071A44]">
        {value}
      </p>
    </div>
  );
  ...
  return (
    <article
      className={`min-w-0 rounded-[24px] border p-4 ${rankStyle} ${
        isMe
          ? 'ring-2 ring-[var(--puzzle-primary)] ring-offset-2'
          : ''
      }`}
    >
      <div className="flex items-center justify-between gap-3">
        ...
        {isMe ? (
          <span className="rounded-full bg-[var(--puzzle-primary)] px-2.5 py-1 text-[10px] font-black text-[var(--puzzle-text-on-primary)]">
            You
          </span>
        ) : (
          <TrophyIcon className="h-5 w-5 text-[#6E6A63]" />
        )}
      </div>
      ...
      <p className="mt-1 text-xs text-[#6E6A63]">
        {entry.weeksCompleted} puzzle
        {entry.weeksCompleted !== 1 ? 's' : ''}{' '}
        completed
      </p>

      <p className="mt-4 font-serif text-3xl text-[#071A44]">
        {entry.totalScore}
      </p>
  ...
  return (
    <div
      className={`overflow-hidden rounded-2xl border ${
        isMe
          ? 'border-[var(--puzzle-primary)] bg-[var(--puzzle-bg-accent)]'
          : 'border-[#E8E0D3] bg-[#FBF8F3]'
      }`}
    >
      <button
        type="button"
        onClick={onToggle}
        className="flex w-full min-w-0 items-center justify-between gap-3 px-3 py-3 text-left sm:px-4"
      >
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-2">
              ...
              {isMe ? (
                <span className="shrink-0 rounded-full bg-[var(--puzzle-primary)] px-2 py-0.5 text-[9px] font-black text-[var(--puzzle-text-on-primary)]">
                  You
                </span>
              ) : null}
            </div>
            ...
            <p className="mt-0.5 text-[10px] text-[#6E6A63]">
              {entry.weeksCompleted} completed
            </p>
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-xs font-black text-[#8A847B]">
            {expanded ? '−' : '+'}
          </span>
        </div>
      </button>
      ...
      {expanded ? (
        <div className="border-t border-[#E8E0D3] bg-white p-3 sm:p-4">
          {entry.weeks.length === 0 ? (
            <p className="rounded-xl bg-[#FBF8F3] p-4 text-center text-sm text-[#6E6A63]">
              No week data yet.
            </p>
          ) : (
            <div className="space-y-2">
              {entry.weeks.map(week => (
                <div
                  key={week.weekNumber}
                  className="grid min-w-0 gap-3 rounded-xl border border-[#E8E0D3] bg-[#FBF8F3] p-3 sm:grid-cols-[1fr_auto] sm:items-center"
                >
                  <div className="min-w-0">
                    <p className="text-xs font-black text-[#071A44]">
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                    ...
                    <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-black text-[#071A44] shadow-sm">
                      {week.totalScore} pts
                    </span>
                  </div>
                </div>
