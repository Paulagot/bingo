...
export interface MovingTargetTapConfig {
  roundType: 'moving_target_tap';
  durationMs: number;
  pathType: 'linear' | 'bounce' | 'arc';
...
export interface PathTraceConfig {
  ...
  laneWidth: number;
  template: 'curve' | 'zigzag' | 'stepped_bend' | 'mirrored_s';
  pathPoints: PathTracePoint[];
  startAnchor: { x: number; y: number };
  finishAnchor: { x: number; y: number };
  scoringSampleCount: number;
  maxPointCount: number;
  roundStartTimestamp?: number | null;
}

export type RoundConfig = TrueCentreConfig | MidpointSplitConfig | StopTheBarConfig | DrawAngleConfig | FlashGridConfig | QuickCountConfig | FlashMathsConfig | LineLengthConfig | BalancePointConfig | PatternAlignConfig | SequenceGapConfig | ColourCountConfig | TimeEstimationConfig | CharacterCountConfig | ReactionTapConfig | MovingTargetTapConfig | PathTraceConfig;
...
export interface ReactionTapSubmission {
  roundId: string;
  playerId: string;
  roundType: 'reaction_tap';
...
export interface MovingTargetTapSubmission {
  roundId: string;
  playerId: string;
  roundType: 'moving_target_tap';
  submittedAt: number;
  clientTargetPosition?: { x: number; y: number };
  tapX: number;
  tapY: number;
}

export interface PathTraceSubmission {
  roundId: string;
  playerId: string;
  roundType: 'path_trace';
  submittedAt: number;
  points: PathTracePoint[];
}
...
export interface MovingTargetTapReveal {
  roundType: 'moving_target_tap';
  pathType: 'linear' | 'bounce' | 'arc';
  startPosition: { x: number; y: number };
  velocity: { x: number; y: number };
...
export interface PathTraceReveal {
  roundType: 'path_trace';
  pathPoints: PathTracePoint[];
  playerPoints: PathTracePoint[];
  laneWidth: number;
...
export interface RoundResult {
  ...
  survived: boolean;
  didSubmit: boolean;
  revealData: TrueCentreReveal | MidpointSplitReveal | StopTheBarReveal | DrawAngleReveal | FlashGridReveal | QuickCountReveal | FlashMathsReveal | LineLengthReveal | BalancePointReveal | PatternAlignReveal | SequenceGapReveal | ColourCountReveal | TimeEstimationReveal | CharacterCountReveal |  ReactionTapReveal
...
export interface RoundIntroPayload {
  ...
  eliminationMode: 'none' | 'percentage' | 'reduce_to_finalists' | 'final';
}
