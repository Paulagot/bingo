// src/components/elimination/EliminationWinnerView.tsx
...
interface Props {
  ...
  // ── Web2 configured prize ──
  prizeSponsor?: string | null;
  prizeDescription?: string | null;
  prizeValue?: number | string | null;

  /**
   * Club default currency code, e.g. EUR / GBP / USD.
   *
   * This MUST be supplied from the club/room data.
   * We deliberately do not default this to EUR.
   */
...
const formatPrizeValue = (
  ...
  // If none was supplied, just show the numeric value.
  if (!currency) {
  ...
  try {
    ...
    // Unknown/invalid currency code:
    // still don't assume EUR.
    return `${numeric} ${currency}`;
  }
};

// ── Web2 prize + sponsor banner ───────────────────────────────────────────────

const PrizeSponsorBanner: React.FC<{
  sponsor?: string | null;
  ...
  const formattedValue = formatPrizeValue(value, currency);
  ...
  return (
    <div style={sp.wrap}>
      ...
      <div style={sp.eyebrow}>
        Winning Prize
      </div>

      {description && (
        <div style={sp.description}>
          {description}
        </div>
      )}
      ...
      {sponsor && (
        <div style={sp.sponsor}>
          <span style={sp.sponsorLabel}>
            Prize sponsored by
          </span>
...
const PrizeInfoCard: React.FC<{
  ...
  const totalPool =
    (totalPlayers * roomData.entryFee) /
...
export const EliminationWinnerView: React.FC<Props> = ({
  ...
  // ── Delay feedback so player can read the result first ────────────────────
  ...
  return (
    ...
    >
      ...
      <div style={s.standings}>
        ...
        <div style={s.list}>
          {finalStandings.map(result => {
            const name =
              playerMap[result.playerId]?.name ??
              result.playerId;
      ...
      {/* ── Host footer: reconciliation - Web2 ── */}
      ...
      {isHost &&
        ...
        feedbackDone && (
          <div style={s.closeFoot}>
            ...
            <p style={s.hostHint}>
              Review payments and approve the final count
            </p>
          </div>
        )}
...
const sp: Record<string, React.CSSProperties> = {
  wrap: {
    ...
    background:
      'linear-gradient(135deg, rgba(255,215,0,0.10) 0%, rgba(255,165,0,0.035) 100%)',
