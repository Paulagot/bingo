...
export const EliminationFinalizeSection: React.FC<Props> = ({
  ...
  const {
    ...
    connect,
    handleFinalize,
  } = useEliminationFinalize({ roomId, hostId, winnerPlayerId, roomData });
  ...
  if (state === 'success') {
    return (
      <div style={s.card}>
        ...
        {txHash && explorerUrl && (
          <a href={explorerUrl} target="_blank" rel="noopener noreferrer" style={s.txLink}>
            View transaction
          </a>
        )}
        {onComplete && (
          <button onClick={onComplete} style={{ ...s.btn, marginTop: '12px' }}>
            Return to lobby
          </button>
        )}
      </div>
  ...
  return (
    <div style={s.card}>
      ...
      {/* Breakdown with actual amounts */}
      <div style={s.breakdown}>
        <div style={s.row}>
          <span style={s.label}>Winner</span>
          <div style={s.amountGroup}>
            <span style={s.amount}>{winnerAmount} {tokenSymbol}</span>
        ...
        <div style={s.row}>
          ...
          <div style={s.amountGroup}>
            <span style={s.amount}>{hostAmount} {tokenSymbol}</span>
        ...
        <div style={s.row}>
          ...
          <div style={s.amountGroup}>
            <span style={s.amount}>{charityAmount} {tokenSymbol}</span>
        ...
        <div style={s.row}>
          ...
          <div style={s.amountGroup}>
            <span style={s.amount}>{platformAmount} {tokenSymbol}</span>
