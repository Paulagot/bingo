...
//
// Public peer fundraiser support page.
// Crypto payments added: remove isCryptoMethod filter, add crypto-fixed-fee step.
//
// Supported flow:
// packs -> details -> payment -> payment-instructions -> confirm
//                            -> crypto-fixed-fee      -> confirm   ← NEW
...
// Crypto uses CryptoFixedFeeStep with a peer-specific confirm endpoint that
// calls confirmPeerOrder() — the same fulfilment pipeline as the Stripe webhook.
...
export default function PeerSupportPage() {
  ...
  useEffect(() => {
    ...
    api.paymentMethods(data.fundraiser.id)
      .then(result => {
        // Crypto is now supported — no filter applied.
  ...
  const goToPayment = () => {
    if (checkoutMode === 'order') {
      // Pack purchases always require name and valid email for confirmation
      // and entry link delivery.
      ...
      // Donations — both screen name and email are optional.
  ...
  async function createDonationAndProceed() {
    ...
    try {
      ...
      // ── Crypto donation: create a pending row, get wallet address,
      // then CryptoTicketDonationStep handles the on-chain send + confirm.
  ...
  async function createOrderAndProceed() {
    ...
    try {
      ...
      // ── Crypto: order is created (pending). CryptoFixedFeeStep sends
      // on-chain then calls /api/peer-support/orders/:orderId/confirm-crypto,
      // which runs confirmPeerOrder() — the same fulfilment pipeline as
      // the Stripe webhook. No separate claim step needed.
      if (isCryptoMethod(selectedMethod)) {
        setHasCopiedReference(false);
        setHasOpenedProviderLink(false);
  ...
  // ── First room ID from cart — used as quote anchor for crypto ────────────
  // The peer quote endpoint only needs a fundraiser ID, but CryptoFixedFeeStep
  // uses useCryptoQuote which still passes roomId in the URL. We override the
  // endpoint entirely via quoteEndpoint prop so this value is unused by the
  // backend; it's kept here so the prop type is satisfied.
  ...
  return (
    <AppShell style={appStyle}>
      ...
      {step === 'crypto-fixed-fee' && selectedMethod && orderId && data?.fundraiser && (
        ...
        >
          <Web3Provider force>
            <CryptoFixedFeeStep
              mode="ticket"
              // roomId is passed to satisfy the prop type but the backend uses
              // quoteEndpoint instead, which resolves currency from the fundraiser.
              roomId={firstCartRoomId || data.fundraiser.id}
              // Peer-specific quote endpoint — uses fundraiser currency directly,
              // no room needed. Add quoteEndpoint prop to CryptoFixedFeeStep /
              // useCryptoQuote if not already present (see diff notes).
              ...
              // Peer crypto confirm endpoint — verifies on-chain, then calls
              // confirmPeerOrder() (same as Stripe webhook path).
      ...
      {/* ── Crypto donation — uses CryptoFixedFeeStep so the fiat amount the
           supporter already entered is shown and quoted, not re-entered.
           confirmEndpoint overrides the default ticket path to hit the
           peer donation confirm route instead. ─────────────────────── */}
...
function PackChoiceCard({
  pack, currency, quantity, onOpen, onAdd, onRemove,
}: {
  pack: any;
  currency: string;
  quantity: number;
  ...
  const soldOut = getPackSoldOut(pack) || availability?.available === false;
  const availabilityMessage = availability?.message || (soldOut ? 'Sold out' : null);
  ...
  return (
    <article
      className={`relative overflow-visible rounded-3xl bg-white p-3 shadow-sm ring-1 transition ${
        featured ? 'ring-[var(--fr-primary)]' : 'ring-slate-200'
      } ${quantity > 0 ? 'shadow-orange-100 ring-2 ring-[var(--fr-primary)]' : ''}`}
    >
      {badge && (
      ...
      <button type="button" onClick={onOpen} className="flex w-full items-center gap-4 pt-2 text-left">
        <div
          className={`grid h-20 w-20 shrink-0 place-items-center rounded-3xl ${
            featured ? 'bg-gradient-to-br from-yellow-100 via-amber-100 to-orange-50' : 'bg-slate-50'
          }`}
        >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <h2 className="max-w-full break-words text-[clamp(1.05rem,4.8vw,1.25rem)] font-black leading-tight tracking-tight text-slate-950">
              {pack.name} — {fmt(pack.price, pack.currency ?? currency)}
            </h2>
          ...
          <div className="mt-2 space-y-1">
            {(rooms.length ? rooms : [{ quantity: 1, prizes: [] } as PackRoomDetails])
              .slice(0, 2)
              .map((room, index) => (
                <div
                  key={`${room.roomId ?? index}`}
                  className="flex min-w-0 items-start gap-2 text-sm font-semibold text-slate-600"
                >
          ...
          {topPrize && (
            <div className="mt-2 flex min-w-0 items-center gap-1.5 text-xs font-black text-[var(--fr-primary)]">
              ...
              <span className="min-w-0 flex-1 truncate">
                ...
                {topPrize.value
                  ? fmt(topPrize.value, pack.currency ?? currency)
      ...
      <div className="mt-3 flex items-center justify-between gap-3 border-t border-slate-100 pt-3">
        <div>
          <button type="button" onClick={onOpen} className="text-sm font-black text-[var(--fr-primary)]">
            Details & prizes
          </button>
          {!soldOut && availability?.message && (
            <p className="mt-1 text-xs font-black text-amber-700">{availability.message}</p>
          )}
        </div>
        {soldOut ? (
          ...
          <div className="flex items-center gap-2 rounded-full bg-orange-50 p-1 ring-1 ring-orange-100">
            <button
              type="button"
              onClick={onRemove}
              className="grid h-9 w-9 place-items-center rounded-full bg-white text-slate-900 shadow-sm"
            >
            ...
            <button
              type="button"
              onClick={onAdd}
              className="grid h-9 w-9 place-items-center rounded-full bg-[var(--fr-primary)] text-white shadow-sm"
            >
          ...
          <button
            type="button"
            onClick={onAdd}
            className="rounded-full bg-[var(--fr-secondary)] px-4 py-2 text-sm font-black text-white"
          >
            Add
          </button>
        )}
...
function PackArtwork({ pack, featured }: { pack: any; featured: boolean }) {
  ...
  if (type.includes('quiz') || game === 'quiz')
    return <Users className={`${className} text-slate-950`} />;
  if (type === 'elimination_entry' || game === 'elimination')
    return <Trophy className={`${className} text-slate-950`} />;
...
function OrderMiniSummary({ cartItems, currency }: { cartItems: CartItem[]; currency: string }) {
  const total = cartItems.reduce(
    (sum, item) => sum + asNumber(item.pack.price) * item.quantity,
    0,
  );
  return (
    <div className="mt-5 rounded-3xl bg-slate-50 p-4 ring-1 ring-slate-100">
      ...
      <div className="space-y-2">
        {cartItems.map(item => (
          <div
            key={item.pack.id}
            className="flex justify-between gap-4 text-sm font-bold text-slate-700"
          >
            <span>
